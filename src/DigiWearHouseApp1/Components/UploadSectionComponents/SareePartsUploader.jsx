...
const SareePartsUploader = ({ formData = {}, onChange = () => {} }) => {
  ...
  // --- FIX 1: Wrap helper functions in useCallback ---
  const getAllPartsUploaded = useCallback(() => {
    // Depend on sareeParts directly from formData for consistency
    const parts = formData.sareeParts || {};
    return Object.values(parts).every((part) => part && part.file);
  }, [formData.sareeParts]); // Depend on the prop

  const handleAutoGenerateCompleteSaree = useCallback(async () => {
    if (!getAllPartsUploaded()) {
      console.log("Not all parts uploaded yet");
      return;
    }
    ...
    try {
      ...
      // Use the stable sareeParts definition from the component's scope
      Object.entries(sareeParts).forEach(([partName, partData]) => {
        if (partData.file) {
          formDataToSend.append(partName, partData.file);
        }
      ...
      if (data.success) {
        ...
        // Use the stable sareeParts definition
        const partUrls = [
          sareeParts.blouse?.url,
          sareeParts.pleats?.url,
          sareeParts.pallu?.url,
    ...
    // --- Add dependencies ---
  }, [getAllPartsUploaded, onChange, sareeParts]);

  // --- FIX 2: Update useEffect dependencies ---
  useEffect(() => {
    const allPartsUploaded = getAllPartsUploaded();
    const noGeneratedViews = !formData.generatedSareeViews;

    if (allPartsUploaded && noGeneratedViews && !generatingComplete) {
      console.log(
        "All 4 parts uploaded, auto-generating complete saree views..."
      );
      handleAutoGenerateCompleteSaree();
    }
  }, [
    getAllPartsUploaded, // Use the memoized function
    formData.generatedSareeViews,
    generatingComplete,
    handleAutoGenerateCompleteSaree, // Add the memoized function
  ]);
  ...
  const clearAllParts = () => {
    Object.keys(sareeParts).forEach((partName) => {
      if (sareeParts[partName] && sareeParts[partName].preview) {
        URL.revokeObjectURL(sareeParts[partName].preview);
      }
    });
  ...
  const getUploadedPartsCount = () => {
    return Object.values(sareeParts).filter((part) => part && part.file).length;
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center p-6 animate-pulse">
        ...
        <p className="text-gray-500 text-xs mt-1">
          AI is creating multiple angles
        </p>
      </div>
    );
  };
    ...
    <div className="max-w-6xl mx-auto p-6 bg-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        {Object.entries(partLabels).map(([partName, partInfo]) => (
          <div
            key={partName}
            className="bg-white rounded-lg border-2 border-dashed border-gray-300 hover:border-blue-400 transition-all duration-200"
          >
            {!(sareeParts[partName] && sareeParts[partName].file) ? (
              <div
                className={`p-8 text-center transition-all duration-200 ${
                  draggedPart === partName
                    ? "border-blue-400 bg-blue-50"
          ...
          <span className="text-sm font-medium text-gray-700">
            ...
            {formData.generatedSareeViews && (
              <span className="ml-2 text-green-600">
                {generatingComplete
                  ? "(Generating...)"
                  : `(+ ${getGeneratedViewsCount()} AI Views ✨)`}
              </span>
            )}
          </span>
          {getUploadedPartsCount() > 0 && (
      ...
      {formData.generatedSareeViews && (
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            {Object.entries(formData.generatedSareeViews)
              .map(([viewType, viewData]) => {
                const viewUrl = formData.generatedSareeUrls?.[viewType];
                const viewLabel = viewLabels[viewType];

                // Handle potential null/undefined viewData
